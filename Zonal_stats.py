...
lama_poly = r"D:\Lama NF\Shp_Lama\polygon.shp"
lama_line = r"D:\Lama NF\Shp_Lama\line.shp"
lama_ndvi = r"D:\Lama NF\2021\ndvi.tif"


def zonal_data(raster_path, shp_path):
    ...
    for feature in source_layer:
        ...
        # Extract the raw NDVI values within the polygon
        #masked_ndvi = np.ma.masked_array(raster_array, mask=~mask)

        df.append({"Feature": feature.GetField('name'), "value": out_values})
    ...
    for h in range(data_frame[{"value"}].shape[1]):
        for j in range(data_frame[{"Feature"}].shape[0]):
            ...
            for item in value:              
                data.append({"name":name, "value":item})
